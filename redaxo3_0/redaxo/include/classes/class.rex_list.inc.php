 ...
 * @package redaxo3 
 * @version $Id: class.rex_list.inc.php,v 1.1 2006/11/10 18:08:28 kristinus Exp $ 
 */ 
...
class rex_list
{
	...
	function rex_list()
	{
		...
		$this->blaettern_head = 
			'<b><a href="###LINK_BACK###"><img src='.$REX["HTDOCS_PATH"].'/pics/back.gif width=24 height=13 border=0></a> '.
			'<a href="###LINK_NEXT###"><img src='.$REX["HTDOCS_PATH"].'/pics/forward.gif width=24 height=13 border=0></a>'.
			' &nbsp; &nbsp; &nbsp; &nbsp; ###LIST_START### - ###LIST_END### of ###LIST_ALL###'.' </b>';
				...
				}else
				{
					...
					for ($k=0;$k<count($this->format[$i]);$k++)
					{
						...
						switch($this->format[$i][$k])
						{
							...
							case("checkbox"):
								$value = "<input type=checkbox name='".$this->format_value1[$i][$k]."' value='".$value."'>";
								break;
